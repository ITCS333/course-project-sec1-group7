...
function createUser($db, $data) {
    ...
    if ($isAdmin !== 0 && $isAdmin !== 1) {
        sendResponse('Invalid admin value', 400);
    }
    ...
    sendResponse('User could not be created', 500);
}
...
function updateUser($db, $data) {
    ...
    sendResponse('User could not be updated', 500);
}
...
function deleteUser($db, $id) {
    ...
    sendResponse('User could not be deleted', 500);
}
...
function changePassword($db, $data) {
    ...
    if (empty($data['id']) || empty($data['current_password']) || empty($data['new_password'])) {
        sendResponse('Missing password fields', 400);
    }
    ...
    sendResponse('Password could not be updated', 500);
}
